...
function mainMenu(person, people) {
  ...
  let displayOption = promptFor(
    person.firstName +
      ...
      person.lastName +
      " " +
      "was the result of the given search criteria. Do you want to know their 'info', 'family', or 'descendants'? Type the option you want or 'restart' or 'quit'",
    autoValid
  );
...
function displayFamily(person) {
  ...
  parentOne = parentOne
    ? data.find((potentialMatch) => potentialMatch.id === parentOne)
    : "";
  parentTwo = parentTwo
    ? data.find((potentialMatch) => potentialMatch.id === parentTwo)
    : "";
...
//     let parentOne = currentPerson.parents[0] ? currentPerson.parents[0] : null;
//     let parentTwo = currentPerson.parents[1] ? currentPerson.parents[1] : null;
//     if (searchId === parentOne || searchId === parentTwo) {
//       childrenString += currentPerson.firstName + " " + currentPerson.lastName + "\n"
//       childrenArray.push(currentPerson)
//     }
//   }
//   alert(`${person.firstName}'s Descendants:
//   ${children} `)
// }
...
function displayDescendants(person) {
  function recurse(target) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      let parentOne = currentPerson.parents[0] ? currentPerson.parents[0] : null;
      let parentTwo = currentPerson.parents[1] ? currentPerson.parents[1] : null;
      if (target.id === parentOne || target.id === parentTwo) {
        childrenArray.push(currentPerson)
      }
    }
 ...
 const displayString = recurse(person)
 alert(`${person.firstName}'s Descendants:
 ${displayString} `)
...
function searchByTrait(people) {
  let trait = prompt(
    "Would you like to search by gender, dob, eye color, weight, height or occupation?"
  ).toLowerCase();
  let result;
  switch (trait) {
    case "gender":
